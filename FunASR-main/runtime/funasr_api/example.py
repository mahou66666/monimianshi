...
def backend_sv_filter_example():
    rcg = FunasrApi(
        uri="ws://127.0.0.1:10095/",
        gateway_url="http://127.0.0.1:8082"
    )

    # enroll prepared blacklist audio
    enroll_res = rcg.enroll_blacklist_file("asr_example.wav", speaker_id="blacklist_user")
    print("enroll_res=", enroll_res)

    # verify first, then run asr only for non-blocked speaker
    result = rcg.rec_file_with_sv_filter("asr_example.wav")
    print("rec_file_with_sv_filter=", result)

    # recognize without speaker filter via Java Controller
    rec_result = rcg.recognize_file_via_controller("asr_example.wav")
    print("recognize_file_via_controller=", rec_result)
...
if __name__ == "__main__":
    ...
    # need local ws asr service and sv-service
    # backend_sv_filter_example()
